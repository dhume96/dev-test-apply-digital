...
export class ReportService {
    ...
    async deletedProductsReport(): Promise<any> {
        ...
        const deletedProductsCount = await this.productModel.countDocuments({ isDeleted: true });

        const report = {
            deletedProductsPercentage: (deletedProductsCount / productsTotalCount) * 100,
        };

        return report;
    }

    async nonDeletedProductsReport(input: ReportInputDto): Promise<ProductPayload[]> {
